...
# creating new variable for match activation date to completion in days
data <- data %>%
  mutate("Little Days Acceptance to Match Activation Date" = as.numeric(difftime(`Match Activation Date`,`Little Acceptance Date`, units = "days")))
...
unique_little_days_acceptance_to_match_activation <- data %>% 
  select("Little ID", "Match ID 18Char", "Little Days Acceptance to Match Activation Date") %>% 
...
merge_test <- left_join(cleaned_date_data, unique_big_days_approved_to_match_date, by = join_by(`Match ID 18Char`, `Big ID`))
```



```{r}
cleaned_date_data <- merge_test
```


```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_big_days_assesment_uploaded_to_match_date, by = join_by(`Match ID 18Char`, `Big ID`))
```
...
```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_big_days_contact_created_to_match_date, by = join_by(`Match ID 18Char`, `Big ID`))
```
...
```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_big_days_enrollment_to_match_date, by = join_by(`Match ID 18Char`, `Big ID`))
```
...
```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_days_match_activation_to_closure_date, by = join_by(`Match ID 18Char`))
```
...
```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_little_days_acceptance_to_match_activation, by = join_by(`Match ID 18Char`, `Big ID`))
```
