...
```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_little_days_acceptance_to_match_activation, by = join_by(`Match ID 18Char`, `Little ID`))
```
...
```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_little_days_application_recieved_to_match_activation, by = join_by(`Match ID 18Char`, `Little ID`))
```
...
```{r}
# merging the big acceptange to match activation in days variable
merge_test <- left_join(cleaned_date_data, unique_little_days_interview_to_match_activation, by = join_by(`Match ID 18Char`, `Little ID`))
```
...
```{r}
write.xlsx(cleaned_date_data, "~/MUDAC/minneMUDAC2025/Data/all_date_variables_cleaned_df.xlsx")
```
